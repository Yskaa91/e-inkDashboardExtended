...
for(let tpvName of changedTPV){
	...
	case 'position':
		if(mobPosition && tpv.position) {	// Update the distance to mob
		...
		if(tpv.nextPoint && tpv.nextPoint.value && tpv.nextPoint.value.position && tpv.position) {	// Update the next track point
			displayNextPoint();
		}
		break;
	...
	case 'track':
		...
		else {	// You need to show the card, even if there is no direction, because the wind and mob can be shown on it
	...
	case 'collisions':
		...
		if(tpv.collisions && tpv.collisions.value){
			let collisions = Object.entries(tpv.collisions.value.vessels).sort(function(a,b){return a[1].dist - b[1].dist;});	// Ride sorting, collisions - an array of arrays of two elements, the first is the former key, the second is the former value
		...
		break;
	case 'mob':
		if(tpv.mob && tpv.mob.value){	// MOB mode is
			if(tpv.mob.value.position){	// This is Geojson, probably from Galadrielmap
				// look for a point indicated as current
				let point;
				for(point of tpv.mob.value.position.features){	// there are not only points, but also linestring
	...
	case 'propRevolutions1':
		...
		if((displayData[tpvName].DOMid == 'leftBottomBlock') && mobPosition) break;	// We will not draw in the lower left corner if the mode is mob
		if((displayData[tpvName].DOMid == 'rightBottomBlock') && tpv.collisions && tpv.collisions.value) break;	// We will not draw in the lower right corner if the danger of a collision
		
		htmlBLock = document.getElementById(displayData[tpvName].DOMid);
		if(!tpv[tpvName] || tpv[tpvName].value === undefined) {	// Null - this engine is stopped.There could be a Display call for all values - for updating the screen
		...
		if(displayData[tpvName].DOMid.includes('ottom')) {	// It is indicated placed in the lower corners
			str += strPropLabel+'<span style="font-size:var(--ltl1-font-size);"><br><br></span>';	// The human name of the engine from Signalk
			str += strPropVal;
			...
			str += strPropVal;
			str += '<span style="font-size:var(--ltl1-font-size);"><br><br></span>'+strPropLabel;	// The human name of the engine from Signalk
		}
	...
	case 'propTemperature1':
		...
		htmlBLock = document.getElementById(displayData[tpvName].DOMid);
		if(!tpv[tpvName] || tpv[tpvName].value === undefined) {	// There could be a Display call for all values - for updating the screen
			htmlBLock.style.display = 'none';	// To turn off events
			break;
		}
		if(tpvName[tpvName.length-1] == '0') {
	...
	default:
		...
		htmlBLock = document.getElementById(displayData[tpvName].DOMid);
		if(!tpv[tpvName] || tpv[tpvName].value === null || tpv[tpvName].value === undefined) {	// There could be a Display call for all values - for updating the screen
			htmlBLock.style.display = 'none';	// To turn off events
			break;
		}
...
if((displayData.nextPoint.DOMid == 'rightBottomBlock') && tpv.collisions && tpv.collisions.value) return;	// We will not draw in the lower right corner if the danger of a clashя
...
let str='';
if(displayData.nextPoint.DOMid.includes('ottom')) {	// It is indicated placed in the lower corners
	str += dist;			
...
if(speed === null){
	while (windMark.firstChild) {	//Remove all symbols from the icon
...
if(oldWind.w25cnt == w25cnt && oldWind.w5cnt == w5cnt && oldWind.w2dt5cnt == w2dt5cnt) return;	// The view of the arrow has not changed
...
for(let i=w5cnt; i--;){	// We draw feathers 5 m/s
...
for(let i=w25cnt; i--;){	// We draw feathers 25 m/s
	upHline(posX);	// draw the upper connecting line, remember the new POSX do not need
	posX = downHline(posX);	// draw the lower connecting line
...
function chkTPV(tpvName,checksTPV) {/* Checks whether TPVNAME*/
